...
export async function POST(request: NextRequest) {
  try {
    ...
    const city = body.request.nlu?.entities?.find((e: any) => e.type === 'YANDEX.GEO')?.value?.city;
    ...
    let filteredEvents = mockEvents.filter(event => {
      ...
      const matchesDate = !datetime || 
                         normalizeDate(event.date.toLowerCase()).includes(normalizeDate(datetime.value).toLowerCase());
    ...
    if (resultEvents.length > 0) {
      ...
      responseText = `Вот мероприятия${cityPart}${datePart}:\n` + 
        resultEvents.map((event, index) => 
          `${index + 1}. ${event.title}. ${event.date} в ${event.location}. ${event.description}`
        ).join('\n');
        
      responseTTS = `Я нашла ${resultEvents.length} мероприятий${cityPart}${datePart}. ` +
